...
Laravelを用いた実践的なWebアプリケーション開発として、代表的な下記の項目を組み合わせた総合的な予約管理システムを構築するために作成しました。
- 認証機能
- 権限管理機能
- 店舗予約（変更）機能
- 口コミ（編集）機能
...
Laravel（Blade）での従来の主要画面から、 段階的にNext.jsを用いたSPA構成へ移行し、 API設計・認証・フロント連携の理解を深めることも進めてます。
...
管理者画面から店舗情報を一括登録できる機能として実装を進めています。
...
### 一般ユーザー機能（一部機能のみ）
- 会員登録
- ログイン　＊認証後、仮で「バックエンド側の店舗一覧画面」に遷移させてます。
- 店舗一覧表示
- 店舗検索
- エリア検索
- ジャンル検索
...
- 今度、fortifyを再設定の予定。入れた後、「メール認証機能」も整備したいです。
